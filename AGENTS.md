...
本文件为 **phpo 项目的最高技术总纲**（最高权威：任何文档、代码、注释、测试与本文件冲突时以本文件为准）。**本文件与 `docs/` 全部文档、代码注释、面向用户的文案一律用大白话写——见 §0.2 规则 38。**当本文件与其他任何文档、注释、口述、历史草案冲突时，**一律以本文件为准**。
...
### 0.1.2 文风：任何文档必须说人话（v2.9.14 追加）

**一句话**：本项目所有文字（本文件、`docs/` 32 篇、`任务工单.md`、`实施顺序.md`、代码注释、以及**给用户看的**界面文案与错误信息）**一律用大白话写**；读者不需要边读边查词典。

**写法固定三步**：

| 步 | 要写什么 | 反例（不合格） | 正例（合格） |
|----|---------|--------------|------------|
| ① 先说这是干什么的 | 一句话给出这件事的**目的或结果** | 「`ExtStatus` 在返回前执行一次实测归一回写快照回流链路」 | 「打开「管理扩展」时，后端到容器里现查一次哪些扩展开着，再把这份结果写回权威库、推给界面」 |
| ② 再说什么时候会错、错了看得见什么 | 用户**在界面上**会得到什么（一行说明 / 弹框 / 日志） | 「否则违反硬红线 4 造成界面失真」 | 「拿不到实测结果时，界面显示上次存库的那一套并写一行『非实时』，不会整片显示成没装」（编号可放在句尾作索引，但不能只写编号） |
| ③ 最后说怎么回来 | 恢复路径：点哪里、改哪里、下一步是否自愈 | 「异常需人工介入」 | 「腾出那个端口或再改一次端口，站点即自愈」 |

**四条不得**：

1. **不得拿术语当解释**——「权威集」「派生态」「落地链」「归一」「幂等」「去帧」「argv」这类本项目自造/借用词，**首次出现必须顺带说清它指什么**；整段只用术语串起来即视为不说人话。
2. **不得拿条款编号当正文**——`见 §5.16.2` 是**索引**，不是解释本身；被指向的那一段也必须自解释。一句话里只有「因为 X 条所以 Y」而读不懂 Y 是什么，即重写。
3. **不得把实现细节写进面向用户的文案**——前端 locales、向导提示、错误信息只说**结果与下一步**（「扩展已启用，站点 5 秒内可访问」），不写函数名、分支名、事件名。
4. **不得为了「通俗」而失真**——说人话**不放松**任何准确性要求：数字仍须命令实测（§0.2 规则 2）、路径仍守 §0.1.1 记法、口径仍与冻结条款一致、该写 `0777` 的不能写成「权限放开」。

**判据（可执行）**：把新写的一段拿给**没参与过当轮讨论的同事**读，他读完能复述「改了什么 / 为什么会错 / 怎么验证」即合格；他需要反问才能读懂的，就是重写的时候。

**例外**：表格里的**字段名、文件路径、函数名、命令**（如 `internal/store/snapshot.go`、`npm run check`）照写——它们是精确指针，不是文风；但**围绕它们的那句话必须读得懂**。
...
| 管理扩展 | `PhpExtensionsModal.vue`（PHP 列表页「管理扩展」） | 该版本容器内**此刻已启用**的扩展集（唯一真值 = 实测 `php -m`，见下「启用态的唯一真值」），经后端回写 `php_extensions` 后取**权威快照** `appState.phpExtensions[version]`（v2.9.14 需求 ⑥：不得是空清单；v2.9.14 追加：不得是「上次请求的目标集」） | `preflight('extensions')` → **提交即 `emit('close')`** → `applyExtensions` → `.then` 里 `await syncState()` → toast；扩展列表写后**拉权威快照归位**，不做本地乐观更新 |
...
- **不做「失败后留着弹窗让用户改一项」**：重开管理弹窗的默认勾选**就是**该版本容器内此刻已启用的扩展集（实测 `php -m` 现取回写后的权威快照，见下「启用态的唯一真值」），改一项重试这条恢复路并未因此变长；而「必须靠弹窗活着才改得对」等于把一次后台任务绑在一个瞬时的视图实例上。
- 安装弹窗（`InstallModal`）同理在 `submitWrite` **之前**即关闭（§5.6.4 边界 ①），不属本条新增改动。

**启用态的唯一真值（v2.9.14 追加，需求：「这里只显示是否已经启用。已经启用就是 on」）**

用户在真机上看到的现象是：管理扩展弹窗里「基础／数据库／缓存 · 序列化／图像 · 文档／网络 · 消息／异步 · 高性能／框架 C 实现／调试」八组中，**明明这个 PHP 版本已经启用的扩展却画成关**——因为界面取的是库里那份**上次 Apply 请求的目标集**，而基座自带的那一批从来没进过这个集合。这不是「同步时机没选好」，是**判据对象取错了**：界面在回答「上次让你选了什么」，用户要的是「现在开着没有」。

| 项 | 冻结口径 |
|----|---------|
| 唯一判据 | 容器内**实测** `php -m` 的模块名集合；`EXT_CATALOG` 每一行的 on/off 按归一后的名字在这份集合里查。**没有第二个判据**——库里旧值、上次提交的目标集、前端本地状态都不参与判定（硬红线 4） |
| 名字归一 | `php -m` 的输出是显示名，须归一到目录名后再比对：大小写（`PDO` → `pdo`、`SQLite3` → `sqlite3`）、`Zend OPcache` → `opcache`；归一表在后端一处收口，**不得**在前端再做一次二次判定 |
| 取用时机 | **每次打开弹窗由后端现取一次**（`ExtensionService.Status` 先实测再回写 `php_extensions`）；「安装成功后同步」是本条的自然特例——安装链路结束时容器在跑，随后任何一次快照回流拿到的就是实测集，**不**为安装单独立一条同步规则 |
| 回流方式 | 实测结果**只经后端 → 库 → 权威快照**回流；前端**不得**直接消费 `php -m` 文本或新增事件名（17 事件名冻结），走的仍是 `state:changed` 的 `phpExtensions`（§5.6.2） |
| 容器未运行 | 退回库里那份，并**明示非实时**（弹窗内一行说明「容器未运行，显示的是上次落库的启用集」，该行期间不可提交）。**绝不**因为拿不到实测就把整片铺成 off——那等于把「不知道」画成「没有」 |

**三档显示（不可省，因为「可取消」与「已启用」是两件事）**：

| 显示 | 含义 | 判据 |
|------|------|------|
| `on · 可停用` | 已启用，且删得起 | 在实测集合内 **且** `conf.d/docker-php-ext-<name>.ini` 存在（真机干净基座只有 `opcache`／`sodium` 两项，见 §5.16.4） |
| `on · 内建不可停用` | 已启用，但**静态编进基座**，本产品的停用机制对它无效 | 在实测集合内 **且** 无 `.so`、无 ini |
| `off` | 未启用 | 不在实测集合内 |

中间档是这条需求的**硬要求**：真机取证（干净基座 `php:8.4-fpm` 与 `phpo/php:8.2`）显示 `php -m` 共 **35** 个模块，其中落在 `EXT_CATALOG` 73 项内的有 **21** 项，而 `extension_dir` 里只有 `opcache.so` 与 `sodium.so`、`conf.d/` 只有 `docker-fpm.ini`、`docker-php-ext-opcache.ini`、`docker-php-ext-sodium.ini` 与 `zz-phpo.ini`（最后那份是**挂载进去的 php.ini**，不是扩展 ini，见 `internal/config/paths.go` 的 `zz-phpo.ini`）——即 **19** 项是静态内建，对它们执行 §5.16.4 的 `rm -f` 是**空操作**。把这类项画成一颗可取消的开关，用户点了「停用」之后它仍然在 `php -m` 里，界面从此常年说谎（§3.4.1 第 4 条：发现矛盾必须报告，不得绕过）。
...
**编译失败还要说出「该装什么」**（v2.9.14 追加，见 §5.16.6）：上面那句只回答「哪个扩展失败了」，而真机上多数失败的真因是**基座里缺一个系统开发包**，用户看不见包名就没法动手。因此同一批日志里再多落一行 `err` 点名缺失的包（含 Debian／Alpine 两种安装名）与一行 `dim` 说明恢复路径，toast 也带上包名。识别与措辞的完整口径在 §5.16.6。
...
> **这条机制的适用边界（v2.9.14 追加）**：`rm -f` 只对**以 ini 加载的可卸载扩展**有效。**静态编进基座**的那些（真机干净基座占目录 **19** 项，见 §5.16.2 三档显示）既没有 `.so` 也没有 ini，对它执行本命令是**空操作**——因此在界面上这类项必须标「内建不可停用」，**不得**给它一颗可点击的停用开关；`removed` 集合里也不应包含它（后端收到即跳过并给一行 `dim` 说明，不静默）。

#### 5.16.5 明确禁止

- ❌ 安装弹窗只列三五个扩展、其余要用户手输（需求 ① 的原始缺陷）；也**不得**在两个弹窗里各维护一份裁剪清单。
- ❌ 管理扩展弹窗的默认勾选不是「本版本容器内**此刻已启用**的扩展集」——判据是实测 `php -m`，不是「上次 Apply 请求的目标集」（§0.2 规则 37 · §5.16.2）。
...
- ❌ 管理扩展弹窗的默认勾选取自本地状态、上次会话残留、库里旧值或空清单——只认**实测 `php -m` 回写后**的权威快照 `phpExtensions[version]`。
- ❌ 让前端直接消费 `php -m` 文本、或为「现取启用态」新增事件名/任务状态（17 事件名与 4 状态冻结）；实测结果必须走 后端 → `php_extensions` → 快照 → `state:changed`（硬红线 4 · §5.6.2）。
- ❌ 把「停用」实现成「卸载/删 `.so`」，或反过来把已停用的扩展画成 on——停用的语义就是**不再加载**（删 `conf.d` ini），`.so` 留在镜像里，开关照样回 off（§5.16.4）。
- ❌ 给静态内建的扩展（真机干净基座 **19** 项，无 `.so` 无 ini）一颗可点击的停用开关——对它执行 `rm -f` 是空操作，点了不动等于界面撒谎；必须显示为 `on · 内建不可停用`（§5.16.2 三档显示）。
- ❌ 拿不到实测（容器未运行 / exec 失败）就把整片开关铺成 off，或不作任何说明——必须退回库里集**并明示「非实时」**，且该期间不可提交。
...
- ❌ 扩展编译失败只说「安装失败」而不点名缺的系统开发包（§5.16.6）；或把表里没有的名字编一个包名报给用户——不认识就原样报名字。
- ❌ 把 `Requested 'x >= 版本' but version of X is …`（包装了、版本太旧）报成「缺系统开发包」，或**代**用户 `apt-get install` / `apk add` 装基座系统包（§0.2 规则 36：只离线扩展包本体，不碰基座系统包）。
- ❌ 停用失败（`rm -f` 一份 ini）也套用「缺系统开发包」的措辞——那是两件不同的事，甩错锅会把用户支到错误方向（§5.16.6 边界 2）。

#### 5.16.6 扩展编译失败要说清「缺的是哪个系统包」（v2.9.14 追加）

**这是干什么的**：扩展在自己的目录里显示装不上，十有八九**不是扩展本身缺失**，而是它编译时要用的那个**系统开发包**没装进 PHP 基座镜像——`configure` 用 `pkg-config` 找它，找不到就直接中止。本节的职责是把那份「找不到」的编译输出翻成人话：**缺的是哪个包、在这个基座上该敲什么命令装、装完点哪里回来**。

**真机取证（四条失败任务同一个形状）**：`<用户数据目录>/phpo.db` 的 `operations` 表里那四条扩展安装失败记录，停在同样两句上：
...
而当时界面上只看得到「扩展 gd 安装失败，本次扩展集未应用」——用户不知道该装什么，只能反复点重试（这也是「换 Alpine 基座能不能修好」这一问的真实答案：**修不好**，缺的从来是同一条「扩展 → 系统包」的知识，两种基座只是包名不同）。

**识别哪些写法、不认哪些（认错了就是说假话）**：

| 写法 | 认不认 | 为什么 |
|------|-------|-------|
| `Package requirements (zlib >= 1.2.11) were not met` | ✅ 认，取出 `zlib` | 括号里可能列多项（`(libavif libwebp)`），逐项去版本约束、转小写、去重 |
| `Package 'zlib', required by 'virtual:world', not found` | ✅ 认，取出 `zlib` | 同一行两种写法都命中时只报一次 |
| `Requested 'libsodium >= 1.0.8' but version of Sodium is 1.0.16` | ❌ **刻意不认** | 这是「包装了、版本太旧」，报成「缺系统开发包」就是假话；用户照提示重装一遍反而更糊涂 |

**包名表（`internal/config/extdeps.go` 的 `extSysPkgs`）**：**28** 个 `pkg-config` 模块名 → **17** 对包名（Debian ／ Alpine 各一列，如 `zlib` → `zlib1g-dev` ／ `zlib-dev`；`freetype2` → `libfreetype-dev` ／ `freetype-dev`）。两侧包名**逐个现取命令验证过**，未凭印象补录：Debian 侧在真基座 `php:8.4-fpm` 内 `apt-get update` 后逐个 `apt-cache policy` 都有 Candidate；Alpine 侧在 `alpine:latest` 内 `apk update` 后逐个 `apk info -a` 都命中仓库；键本身取自「装齐这批开发包之后」容器内 `pkg-config --list-all` 的真实输出与上面那四条失败日志的原文。**表里没有的名字照样原样报给用户**（`ExtSysPkgHint` 只回那个名字）——宁可少说一种包名，也不得编一个不存在的包。装齐这批包后 `docker-php-ext-install gd` 在真机由失败变成功（退出码 0），这是这张表有效性的直接证据。

**错的时候用户看得见什么**（`extFailed`，§5.16.3 的失败点名之上再加两行）：

| 位置 | 文本 |
|------|------|
| 抽屉一行 `err` | `扩展 gd 缺编译要用的系统开发包: zlib（Debian: zlib1g-dev ／ Alpine: zlib-dev）` |
| 抽屉一行 `dim` | 「phpo 不代装系统包：先在**正在运行的这个 php 容器里**装上上面这些包（Debian 基座 `apt-get install -y`、Alpine 基座 `apk add`），再点一次「应用并重建」——装进容器的那一份会随扩展镜像一起固化，下次不用再装。」 |
| toast（短消息直出） | `扩展 gd 安装失败（缺系统开发包 zlib），本次扩展集未应用` |

**怎么回来**：容器还在跑时 `Apply` 不会重建它（`ensureRunning` 见运行中即直接返回），所以「先进容器装包 → 回界面点「应用并重建」」这条路是真能走通的；第二次编译不再报缺包，且装进去的开发包随 `docker commit` 一起固化进 `phpo/php:{version}`。

**三条边界（不得越）**：

1. **phpo 不代装系统包**——不 `apt-get install`、不 `apk add`、不改用户基座的包管理状态。这既是本轮未获授权的越界，也是 §0.2 规则 36 的既有口径（离线缓存只留扩展包本体，**不为基座系统包造缓存槽位**）；本节只把失败**说清楚**。
2. **停用失败不甩锅系统开发包**——停用只是 `rm -f` 一份 ini（§5.16.4），跟系统包无关。因此该路径调 `extFailed` 时**显式传空**的缺失集，消息仍是「扩展 {name} 停用失败，本次扩展集未应用」；否则一次不相关的 `rm` 失败会被说成「缺包」，把用户支到错误方向。
3. **识别不新增任何探测**——用的就是 §0.2 规则 26 那条已经逐行进日志的编译输出（`extLogWriter` 在写每一行时顺手喂给 `config.ExtMissingDepNames`），**不额外跑 `pkg-config`、不加只读探针、不建「扩展 → 候选包」的第二张表**（那张表要逐扩展真机验证才敢写，是另一轮的事）。事件名、任务状态、快照字段、i18n 键**一个未增**。

**落地登记**：`internal/config/extdeps.go`（`extSysPkgs` / `ExtSysPkgFor` / `ExtSysPkgHint` / `ExtMissingDepNames`，识别用的两条正则与「只收 pkg-config 名形状」的 `depNameRe`）→ `internal/service/extension_service.go`（`runInContainer` 回收并返回去重排序后的缺失名、`extLogWriter.emit` 逐行识别、`extFailed` 落上面那两行并决定 toast 文本）。用例：`internal/config/extdeps_test.go`（真机原文形状的识别 · 「版本太旧」那一族的负向断言 · 表自身形状与「不认识只回原名」）+ `internal/service/extension_service_test.go`（`TestExtension_Apply_FailureNamesMissingSysPkg` · `TestExtension_Apply_DisableFailureDoesNotBlameSysPkg`）。
...
| 扩展启用态现取 `php -m`（v2.9.14 追加，§5.16.2 / §0.2 规则 37） | `internal/service/extension_service.go`（`Status`：`App.ExtStatus` 门面 → `php -m` + `ls -1 conf.d` 两条 argv 只读探针 → 归一显示名（`config.NormalizeExtName`）→ `store.SetPHPExtensions` 回写**实测集** → `state:changed`；容器未运行 / 探针失败 / 实测为空则退回库里集并标 `Live=false`，不写库不发事件）→ `internal/store/snapshot.go` 的 `PHPExtensions` | `PhpExtensionsModal.vue`（`api/extension.ts` 的 `extStatus(version)` 只回 `live`／`builtIn`；启用集仍只读快照 `app.phpExtensions[version]`，基线过滤到「目录内 ∩ 非内建」，用户手改才落 `edits`。**旧的「打开时一次性取本地数组」与「提交后本地改写 `app.phpExtensions`」两处已删除**）+ **三档显示**（`on · 可停用` ／ `on · 内建不可停用`（虚线压暗、点不动、不进提交集）／ `off`）+ 非实时标注行 `ext.nonLive` + 「应用并重建」在 `!live` 时禁用 |
...
- [ ] 安装期是否默认勾选 `commonExts(version)`（11 项）？管理弹窗默认勾选是否等于**本版本容器内此刻已启用的扩展集**（判据 = 实测 `php -m`，**不是**上次请求的目标集，§0.2 规则 37）？
...
- [ ] 编译失败且 `configure` 点名的缺失项在包名表里时，是否**再多落一行 `err`**（包名 + Debian／Alpine 两种安装名）**和一行 `dim`**（怎么回来）？toast 文本是否也带上包名？（§5.16.6）
- [ ] 是否**没有**代用户装系统包（容器里没有 `apt`／`apk` 调用）、**没有**把 `Requested '…' but version of X is …` 这一族版本过旧报成「缺包」、**没有**给停用失败套用缺包措辞？（§0.2 规则 40）
- [ ] 本轮是否**没有**为识别缺失包新增探针／事件名／任务状态／快照字段／文案键（只读已流过的编译输出）？
...
- [ ] 管理扩展弹窗的默认勾选是否取自**权威快照** `appState.phpExtensions[version]`（不是空清单 / 本地残留）？而这份快照的值是否等于**容器内实测 `php -m`**（不是上次 Apply 请求的目标集，§0.2 规则 37）？
- [ ] 三档显示是否齐备（`on · 可停用` ／ `on · 内建不可停用` ／ `off`）？静态内建的 **19** 项是否**没有**被画成可取消开关？
- [ ] 容器未运行 / 实测失败时是否退回库里集**并明示非实时**（而不是整片 off）？该期间是否不可提交？
- [ ] 实测启用态是否**只**走 后端 → `php_extensions` → 快照 → `state:changed`（前端未直接消费 `php -m` 文本、未新增事件名或任务状态，硬红线 4）？
...
- [ ] 本轮写进 `AGENTS.md` / `docs/` / `任务工单.md` / `实施顺序.md` 的每一段，是否**第一句就说明「这是干什么的」**，而不是先铺条件、字段名或实现过程？
- [ ] 说清了**什么情况下会错**、错的时候用户在界面上看得见什么吗？（不是只写「必须 X」，而是「不 X 会怎样」）
- [ ] 说清了**怎么回来**吗？（恢复路径：用户点哪里、或哪一步会自愈）
- [ ] 内部词（「权威集」「派生态」「落地链」「归一」「幂等」「去帧」「argv」……）**首次出现时是否顺带说清它指什么**？有没有整段只用术语写、不读总纲就读不懂？（这些词**不禁用**——要求是「出现时说清指什么」，不是「改用别的词」；§0.1.2 第四条同样禁止为了通俗而把 `0777` 写成「权限放开」。）
- [ ] `见 §x.y` 这类编号是否**只当索引**——被指向的那一段本身自解释？有没有出现「正文只写编号、解释全在另一节」以至于当场读不下去的情况？
- [ ] **面向用户的文案**（前端 `locales/`、向导提示、错误信息、`docs/用户手册.md`）是否避开了内部实现细节（函数名、分支名、事件名），只说结果与下一步？
...
> 用户看到的改动是：打开「管理扩展」不再是上次勾选的那一套，而是这个 PHP 版本此刻容器里开着的那一套——基座自带的
> `curl`／`mbstring`／`openssl` 等终于显示为亮，其中**静态编进 PHP 本体的 19 项**画成虚线压暗并带「内建」标签、点不动
> （对它执行删 ini 是空操作，给一颗可取消的开关等于界面撒谎）；PHP 版本没在运行时，弹窗显示库里那份并写一行
> 「没能在容器里查到……这次不能提交，先启用该 PHP 版本再重新打开」，**不会**把整片铺成熄灭。
>
> **实现要点**（三处，其余口径不变）：① 后端 `internal/service/extension_service.go` 的 `Status` 发**两条只读 argv 探针**
> （`config.ExtLoadedProbeCmd` = `php -m`、`config.ExtIniListCmd` = `ls -1 conf.d`；argv 不经 shell，硬红线：容器内命令不得拼 shell 字符串），
> 显示名经 `config.NormalizeExtName` 归一（`PDO`→`pdo`、`Zend OPcache`→`opcache`）后**只在与库里不同才**回写 `php_extensions`
> 并广播快照（每次开弹窗都重发同一事实即刷屏，§5.19.4 同口径）；容器没跑／探针失败／实测为空三种一律退回库里那份并标
> `Live=false`，**不写库、不发事件**——伪造一份没人实测过的权威值比显示旧值更糟。② 门面用 `App.ExtStatus` **替换** `App.ExtList`
> （一进一出），故 §4.1／§11.3 的「**68 = 70 − ServiceStartup/ServiceShutdown 两颗钩子**」复测仍成立
> （`grep -c '^func (a \*App) [A-Z]' app.go` = **70**、`frontend/bindings/phpo/app.ts` 的 `export function` = **68**），
> 上一条留下的「届时须重新推导」因此作废。③ 前端 `PhpExtensionsModal.vue` 删掉那两处被点名的缺陷
> （打开时一次性取本地数组、提交后本地改写 `app.phpExtensions`——后者违反硬红线 4），改为**基线只读快照**且过滤到
> 「目录内 ∩ 非内建」、用户手改才落 `edits`；`live`／`builtIn` 只随这次请求/响应走、不进快照（不是第二份权威，
> **17 事件名与后端 4 任务状态一个未增**）。服务卡片那处计数 `ServiceView.extCount` 同步只数目录内条目——快照现在是
> 实测全集，含 `Core`／`date` 这类扩展目录管不到的名字，照直数会比弹窗对不上。`Apply` 结尾落的也是**重建后实测集**。
>
> **同源同步**（§13 第 4 步）：`AGENTS.md` §0.1.2 示例与 §5.16.2「取用时机」格的 `ExtList`/`List` 改指 `Status`、
> §6 映射表一行改写、§9 **R105** 的「代码尚未落地」改为「已落地」；`docs/{接口契约,状态同步,界面规格,编码行为准则,CHANGELOG}.md`。
> **验真**：`gofmt -l .` 无输出、`go vet ./...`、`go build ./...`、`go test ./... -count=1` 全绿；五项门禁全过
> （i18n zh-CN / en-US 各 **623** 键且集合相等——本轮新增 `ext.builtin`／`ext.builtinTip`／`ext.nonLive`／`ext.fetching` 四键两侧同步、
> 模板 golden 空 diff、Docker 命名、缓存清单字段、扩展 **73** 项分类对账）；`vue-tsc --noEmit` EXIT=0。
> 新增用例 `internal/service/extension_status_test.go` **9** 条（`php -m` 现取归一回写 · 容器未运行退库 · 探针失败退库 ·
> 实测为空退库 · ini 清单失败即非实时 · 幂等不重复回写 · 内建项进 `BuiltIn` · `Apply` 落实测集 · 内建项落 `removed` 时跳过并聚合 `dim`）。
> **真宿主 GUI 未走查**（三档显示与「非实时」那行的点击级验收待用户实机确认）；**未 commit**（等指令）。
>
> **v2.9.14 追加（未发布版本内折叠，不另计版本号）· 管理扩展弹窗的开关 = 「此刻启用了没有」**：
> 用户现象与裁决经过三轮收窄，最终钉成一句：**「这里只显示是否已经启用。已经启用就是 on」**——① 「有些扩展对应的 php 版本
> 已经安装了，这里却没有反馈出来，导致用户无法直观判断当前已经安装了哪些扩展」；② 我先把它理解成「能不能用」，被连续两次
> 纠正为「**已经安装了这个扩展就是 on，没有就是 off**，而不是能不能用」；③ 再补一条边界「**停用就不加载被停用的扩展，
> 而不是卸载删除**」。用户要求「把这个需求明明白白写进总纲」。
>
> **根因不是同步时机，是判据对象取错了**（这一点必须先说清，否则实现会只做「换个时机查库」）：界面当时的取值链是
> `internal/service/extension_service.go` 的 `Apply` 末尾 `store.SetPHPExtensions(version, enabled)` —— 写进去的是**本次请求的
> 目标集**，而 `frontend/src/components/business/PhpExtensionsModal.vue` 在打开时一次性取 `app.phpExtensions[version]` 作
> `originalExts`、并在提交后**本地改写**同一个字段（该处另违反硬红线 4）。于是基座自带的那一批扩展**从来没进过这个集合**，
> 它们在界面上就永远是 off。用户要的「每次点管理扩展就用 `php -m` 查出来对齐」与「安装成功后同步」两个时机之问，现冻结为
> **打开弹窗时由后端现取一次**，并把「安装成功后同步」判为它的**自然特例**（安装结束时容器在跑，随后回流的就是实测集），
> 不为安装单独立规则。
>
> **真机只读取证**（`docker exec` 只跑 `php -m` / `php -i` / `ls` / `grep`，未改动用户任何容器；期间发现原先在跑的
> `phpo-php-8.0` 已不在，改用实际在机的 `phpo-php-8.4` 与 `phpo-php-8.2`）：`php -m` 出 **35** 个模块，落在
> `EXT_CATALOG` 73 项内的有 **21** 项；`extension_dir` 里只有 `opcache.so` 与 `sodium.so`；`conf.d/` 只有
> `docker-fpm.ini`、`docker-php-ext-opcache.ini`（`zend_extension=opcache`）、`docker-php-ext-sodium.ini`
> （`extension=sodium`）与 `zz-phpo.ini`（**这份是挂载进去的 php.ini，不是扩展 ini**，见
> `internal/config/paths.go`，不得误当作可删项）——即 **19** 项静态内建，**无 `.so` 无 ini，对它执行 §5.16.4 的
> `rm -f` 是空操作**。这条事实逼出一个不能回避的显示口径：若把这 19 项画成可取消的开关，用户点「停用」后它仍在
> `php -m` 里，界面从此常年说谎，因此冻结**三档显示**（`on · 可停用` ／ `on · 内建不可停用` ／ `off`），
> 并要求实测名字**归一**后再比对（`PDO`→`pdo`、`SQLite3`→`sqlite3`、`Zend OPcache`→`opcache`）。
>
> **落条款**：§0.2 新增规则 **37**（故意续在 36 之后不重编号）；§0.3 新增 **2** 行（启用态唯一判据 = 实测 `php -m`／
> 干净基座已启用 **21** 项 · 其中 **19** 静态内建 · **2** 项可停用）；§1.1 结论表「PHP 扩展目录」行补开关语义；
> §5.16.2 表格「默认勾选」一格改写 + 新增子块「**启用态的唯一真值**」（判据/归一/取用时机/回流方式/容器未运行 五格表 +
> 三档显示表 + **现状登记**段——明写本节是**待落地的冻结要求**，并列出三处现存代码落点，避免总纲宣称已完成的事实不存在）；
> §5.16.4 补「这条机制的适用边界」注；§5.16.5 改写 **2** 条禁止项并新增 **5** 条；§6 映射表新增一行；§9 新增 **R105**；
> §12.9 一项改写、§12.10 新增 **4** 项自查；底部摘要新增一条 bullet。**同时覆盖两处旧表述**：头部「扩展弹窗生命周期」
> 那条与上文 v2.9.14 变更段（约第 2682 行）里的「重开管理弹窗的默认勾选**就是**上次成功应用的权威集」——该句现读作
> 「= 实测启用态回写后的权威快照」，旧措辞「上次成功应用的目标集」即本节所禁止的那种取值。
>
> **同源同步**（§13 第 4 步）：`docs/界面规格.md`（管理扩展弹窗三档显示 + 非实时标注）、`docs/接口契约.md`
> （`ExtensionList`/`ExtApply` 的启用态语义与新增的现取能力）、`docs/状态同步.md`（`phpExtensions` 的取值链改为
> 「实测 → 回写 → 快照」）、`docs/用户手册.md`（管理扩展段、提交即关窗段、FAQ 行各一处）、`docs/CHANGELOG.md` 未发布段。
>
> **本轮纯文档 + 只读取证，未改任何 Go/Vue 代码**，因此未跑编译与门禁；**总纲版本仍为 v2.9.14**（未发布版本内折叠，
> 不另起 v2.9.15）。**实现时须一并注意**：现取启用态要新增一个后端能力，因此 §4.1／§11.3 的
> 「**68 个绑定方法 = 70 个导出方法 − 两颗生命周期钩子**」届时必须按真实生成的 `frontend/bindings/phpo/app.ts`
> 重新推导，不得沿用现值（本节已在 §5.16.2「现状登记」注明）。
> **明确未改**：8 条硬红线原文、三段式写操作、**17 个事件名一个未增**（复用 `state:changed` 的 `phpExtensions`）、
> 后端任务状态 **4** 个、preflight **19** action 与 NEEDS_HOME **17**、`pkg/errs` **27** 码、门禁 **5** 项、
> 扩展目录 **73**·**8** 组·常用 **11** 项（本节只改这些项的**显示判据**，未改目录本身）、§5.16.1 唯一目录与
> `check-ext-catalog.go` 对账、§5.16.4「停用 = 删 ini」这条机制本体（只补了它的适用边界）、§5.16.3 执行期日志契约、
> 每版本两镜像槽位、§5.15 三根互斥唯一、i18n 两侧各 **619** 键（实现时新增文案键须两侧同步）、密码／版本／域名／端口策略、
> 冻结原型 SSOT（`前端唯一界面来源.txt` 与 `index.html`）与 `base.css`、前端全部代码。
> **真宿主 GUI 未走查**（本轮无代码改动；`on/off` 三档与「非实时」标注待实现后实机点击验收）。
>
> **v2.9.14 追加（未发布版本内折叠，不另计版本号）· 新增 §0.1.2「文风：任何文档必须说人话」（§0.2 规则 38）**：
> 用户指令原文：**「在最高最高最高最高最高最高总纲加一句，任何文档必须用大白话说人话。」** 六个「最高」是在指定落点——
> 这条要挂在**本文件这一层**（最高权威），不是塞进某篇派生文档。本条不是新发明，是把用户在多轮里反复给出的纠正
> （「说人话」「把需求用大白话描述给我」）**固化成规则**：此前这些纠正只落在具体那一轮，下一轮又漂回术语体。
>
> **落条款**（四处 + 一处自查）：① **§0.1.2 新节**——写法固定三步（**先说这是干什么的 → 再说这种错在界面上看得见什么 →
> 最后说怎么回来**），配一张反例／正例对照表（正例用「打开『管理扩展』时，后端到容器里现查一次哪些扩展开着」这种句子），
> 四条不得（不得拿术语当解释／不得拿条款编号当正文／不得把实现细节写进给用户看的文案／**不得为了通俗而失真**），
> 判据是**可执行**的：拿给没参与过当轮讨论的同事读，他能复述「改了什么 / 为什么会错 / 怎么验证」即合格；
> 并明确**例外**——字段名、文件路径、命令照写（它们是精确指针，不是文风），但包住它们的那句话必须读得懂；
> ② §0.2 规则 **38**（故意续在 37 之后不重编号）；③ §0.1 效力声明补一句并把「最高项目总纲」写成「最高技术总纲 +
> 冲突以本文件为准」；④ 头部条款区在「路径记法」之前新增一条「**文档必须说人话**」；⑤ §12.11 新自查 **8** 项；
> ⑥ 底部摘要在「编码准则」后新增一条。
>
> **本条自身即按本条写**：§0.1.2 三步表里的正例就是本轮刚改的 `docs/用户手册.md` 那三句（「每一颗开关只回答一个问题」
> /「重开弹窗显示的是此刻容器里实测到已启用的那一套」），不是新造的示例。
>
> **同源同步**（§13 第 4 步）：`docs/编码行为准则.md`（§3.4 的派生文档，补「说人话」一节，含三步写法与判据）、
> `docs/CHANGELOG.md` 未发布段。**`docs/` 其余 31 篇与各文档页眉的「冲突以 AGENTS.md 为准」口径不变**——本条只加文风要求，
> 不改变任何既有行为口径；旧文档里那三类内部词（「权威集」「派生态」「落地链」）**不批量返工**（那是另一轮的工作量，
> 且改写历史留痕有失真风险），本轮只把它们钉为「首次出现须说清指什么」。
>
> **本轮纯文档，未改任何 Go／Vue／配置代码**，因此未跑编译与五项门禁；**总纲版本仍为 v2.9.14**（未发布版本内折叠，
> 不另起 v2.9.15）。**明确未改**：8 条硬红线原文、三段式写操作、**17 事件名**／**4 任务状态**、preflight **19** action 与
> NEEDS_HOME **17**、`pkg/errs` **27** 码、门禁 **5** 项、扩展目录 **73**·**8**·**11**、i18n 两侧各 **619** 键（本轮无新增文案键）、
> live 用例 **11**、迁移 **8**、`docs/` **32** 篇（未新增文件）、`app.go` 的 **68 = 70 − 两颗钩子**、§0.1.1 路径记法原文、
...
>
> **v2.9.14 追加（未发布版本内折叠，不另计版本号）· 扩展编译失败要说清「缺的是哪个系统包」（§5.16.6）**：
> 用户现象是**「php 扩展一直安装失败」**，并且猜「是不是换个带 `apk` 的基座（Alpine）就好了」。答案是**否**——本轮
> 从用户自己那份运行中的库（`<用户数据目录>/phpo.db` 的 `operations` 表，**只读**查询，未改动任何一行）取出四条真实
> 失败，逐条读 `configure` 的输出后收口为一句：**失败的真因是基座镜像里没有编译要用的系统开发包，与基座是 Debian
> 还是 Alpine 无关**（换基座只是换一套包名，缺的东西一样缺）。此前界面上看得见的是「扩展 gd 安装失败，本次扩展集
> 未应用」——这句话没错，但用户读完**不知道下一步该做什么**，只能反复点同一颗按钮再失败一次。
>
> **本轮做的就是把那句话补全**：在点名扩展的那行 `err` 之后，同一批日志**再多落一行 `err`**（缺的包名 + 两种安装名，
> 如 `zlib（Debian: zlib1g-dev ／ Alpine: zlib-dev）`）**和一行 `dim`**（怎么回来：先进**正在运行的这个 php 容器**装上它，
> 再点一次「应用并重建」——装进容器的那一份会随 `docker commit` 一起固化进 `phpo/php:{version}`，下次不用重装）；
> toast 也带上包名（`扩展 gd 安装失败（缺系统开发包 zlib），本次扩展集未应用`）。识别只读**已经流过**的编译输出，
> 因此**不新增探针、不新增事件名、不新增任务状态、不新增快照字段、不新增文案键**（17 事件名／**4** 状态／i18n 两侧各
> **642** 键一律未动）。
>
> **三条刻意划下的边界**（比实现本身更重要，否则下一轮会被「顺手加强」掉）：**① phpo 不代装系统包**——不在容器里跑
> `apt`／`apk`，这与 §0.2 规则 36 早已排除的「不为基座系统包造缓存槽位」同源，且替用户在他机器上装东西不是本产品的
> 职责（§0.2 规则 15「用户是程序员」的另一面：告诉他缺什么，比替他做完更符合定位）；**② 版本过旧那一族刻意不认**——
> `configure` 还有另一种写法 `Requested 'libsodium >= 1.0.8' but version of Sodium is 1.0.16`，它说的是包**在**、
> 版本不合，若一并报成「缺包」等于界面在撒谎（§3.4.1 第 4 条：发现矛盾必须报告，不得绕过），因此**没有**为这一族
> 做任何识别，并配了负向用例锁死；**③ 停用失败不甩锅**——停用只是 `rm -f` 一颗 ini，与开发包毫无关系，
> 该调用点显式传 `nil`，不得套用缺包措辞。
>
> **落条款**：头部新增一条「**扩展编译失败要说清缺哪个系统包**」；§0.2 新增规则 **40**（故意续在 39 之后不重编号）；
> §0.3 新增 **1** 行（包名表规模 **28** 个 `pkg-config` 名 → **17** 对 Debian/Alpine 包名）；§5.16.3「失败点名」段
> 补一段指针；**§5.16.6 新节**（干什么 · 真机取证 · 三行识别表 · 包名表 · 错的时候看得见什么 · 怎么回来 · 三条边界 ·
> 落地登记）——**以 5.16.6 追加而不重编号**，因为 §5.16.5 被本文件 8 处（§9 R103/R105、底部摘要、`docs/CHANGELOG.md`
> 等）按号引用，改编号等于把那些引用全部改坏；§5.16.5 补 **4** 条禁止项；§4.1 目录树补 `config/extdeps.go`；
> §9 **R107**；§12.9 补 **3** 项自查；底部摘要一条。
>
> **随本轮一并校正的计数**：与包同目录 `*_test.go` **88 → 89**（新增 `internal/config/extdeps_test.go`；
> §4.1 与 §11.3 各一处，`find . -name '*_test.go' -not -path './test/*' | wc -l` 现取）。包名表规模 **28/17** 由
> python 正则现取，并在 Alpine 与 Debian 两种基座上用 `pkg-config --list-all` 逐项验过名字确实存在；**不改动用户
> 任何在跑的容器**（只在一次性容器里 `docker-php-ext-install gd` 取证退出码 0）。
>
> **验真**：`gofmt -l .` 无输出 · `go vet ./...` · `go build ./...` · `go test ./... -count=1` 全绿 · 五项门禁全过
> （i18n zh-CN / en-US 各 **642** 键且集合相等——本轮**无新增文案键**；模板 golden 空 diff；Docker 命名；缓存清单
> 字段；扩展 **73** 项分类对账）。用例：`internal/config/extdeps_test.go` **3** 条（真机日志形状的识别 · 表形状 ·
> `ExtSysPkgHint` 的归一与不认识）+ `internal/service/extension_service_test.go` **2** 条（安装失败点名缺包的 toast
> 原文与两行日志 · 停用失败不甩锅）。
> **仍欠的那一件**：**live 端到端未跑**——「装一个缺包的扩展、看到那三行、进容器装上再点重建即成功」需要 Docker +
> 外网（`apt`/`apk` 都要拨网络）且会写到用户自己的容器，本机离线且按约定不碰在跑环境，故只由单测假件与真机日志
> 样本背书；**真宿主 GUI 未走查**（那三行在原生窗口里的可读性待用户实机确认）。**未 commit**（等指令）。
> **明确未改**：8 条硬红线原文、三段式写操作、**17 事件名一个未增**／**4 任务状态**、preflight **20** action 与
> NEEDS_HOME **18**、`pkg/errs` 码数（含其 28≠27 的待裁漂移）、门禁 **5** 项、扩展目录 **73**·**8** 组·常用 **11** 项、
> §5.16.1 唯一目录与 `check-ext-catalog.go` 对账、§5.16.2 三档显示与「停用 = 删 ini」的判定口径（本轮只补失败文本，
> 未改任何启用/停用行为）、§5.16.3 执行期日志契约其余各行、每版本两镜像槽位、扩展包本体离线化（规则 36 一字未动，
...
- 扩展编译要说清缺哪个系统包（v2.9.14 追加）：**编译失败时除了点名扩展，还要一行 `err` 说出缺的系统开发包（给 Debian／Alpine 两种安装名）+ 一行 `dim` 说怎么回来（先进正在运行的 php 容器装上它，再点「应用并重建」，装进容器的那一份随扩展镜像一起固化）· phpo 不代装系统包（不在容器里跑 `apt`／`apk`，§0.2 规则 36 已排除）· `Requested '…' but version of X is …` 那一族版本过旧刻意不认成缺包（报成缺包等于界面撒谎）· 停用失败不套用缺包措辞 · 只读已流过的编译输出，不新增探针／事件名／任务状态／快照字段／文案键（见 §0.2 规则 40 / §5.16.6）**
