...
17. **禁止把端口硬编码为 80**：站点端口走「默认 80 + 被占用顺延 1–65535 首个可用（不报错）+ 用户可指定任意端口」逻辑。
...
| 默认站点端口 | **80** | 本项目策略 |
| 站点端口顺延范围 | **1–65535 首个可用** | 占用不报错、无窗口上限 |
| 服务端口冲突 | **报错 portInUse** | 仅站点端口自动顺延 |
...
| 端口策略 | 站点端口默认 80，占用则顺延 1–65535 首个可用（不报错）；服务端口占用报错；用户可指定任意端口 |
...
**核心策略**：站点端口默认 80，被占用则在 1–65535 内顺延首个可用端口，**不报错、无窗口上限**；用户可指定任意端口。服务端口（安装/改配置）占用仍报 `portInUse` 错误，不自动顺延。
...
**站点端口默认 80，被占用则在 1–65535 内顺延首个可用端口，不报错、无窗口上限；用户可指定任意端口（1–65535）。**

**服务端口**（安装服务 / 改配置，非顺延路径）占用时仍报 `portInUse` 错误，由用户自行处理，不自动顺延。
