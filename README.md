...
| 设置 | 布局（侧边栏宽度、日志面板高度）、界面缩放、语言、托盘偏好、检查更新 |
...
| 内容 | 位置 |
|------|------|
| `config.yaml`（路径、密码、端口、数据目录、发布源） | 系统应用数据目录里的 `phpo/`：Linux `~/.config/phpo/`，macOS `~/Library/Application Support/phpo/`，Windows `%APPDATA%\phpo\` |
...
| 脚本 | 锁死什么 |
|------|---------|
| `check-i18n-keys.go` | 中英两套文案**键集合相等**（当前各 619 键）、无重复 |
...
│   ├── preflight/          # **唯一裁决层**（19 个 action；UI 校验只是即时反馈，最终判这里）
...
├── frontend/src/           # Vue 3.5 + TS 5 + Vite + Pinia：12 视图 / 20 业务组件 / 17 api / 8 store
...
| 扩展点了「应用并重建」，弹窗一下就没了 | 正常。那是几十秒的后台任务，提交即关窗，进度和失败全在日志抽屉。下次重开「管理扩展」，默认勾选就是上次**成功应用**的那一套 |
